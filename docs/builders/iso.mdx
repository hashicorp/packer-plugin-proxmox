...
- `efi` (object) - Set the EFI storage location. This need to be set if you use ovmf uefi:
  ...
  - See https://pve.proxmox.com/wiki/Qemu/KVM_Virtual_Machines#qm_bios_and_uefi for details

- `machine` - (string) - Set themachine type. i440fx or q35.

- `replace_existing` - (bool) - If existing VM's or templates with the same name as the template name should be removed prior to creation.
  Defaults to `false`.
